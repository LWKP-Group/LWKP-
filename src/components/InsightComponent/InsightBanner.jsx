...
export default function InsightBanner() {
  ...
  const bgImage = insight?.featured_image;
  const title = insight?.title?.rendered || "Insights";
  const description =
    insight?.acf?.main_description || "Every space we design carries memory forward — alive with meaning.";
