...
import {
  fetchphilsophyPagePosts,
  selectphilsophyPagePosts,
  selectphilsophyPageLoading,
} from "@/store/slices/philosophyPageSlice";
...
export default function PhilosophyBanner() {
  ...
  const philosophy = posts[0];
  const bgImage = philosophy?.featured_image || "";
  const title = philosophy?.title?.rendered || "Philosophy";
  const description =
    philosophy?.acf?.main_description || "Every space we design carries memory forward — alive with meaning.";
