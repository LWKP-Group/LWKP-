...
export default function StoryBanner() {
  ...
  const story = posts[0];
  const bgImage = story?.featured_image;
  const title = story?.title?.rendered || "Story";
  const description = story?.acf?.description || "Every space we design carries memory forward — alive with meaning.";

  return (
    <motion.section
      className="row mainbanner uppercase-class"
