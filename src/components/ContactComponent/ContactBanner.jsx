...
export default function ContactBanner() {
  ...
  // First post
  const contact = posts[0];
  const bgImage = contact?.featured_image;
  const title = contact?.title?.rendered || "Contact Us";
  const description =
    contact?.acf?.main_description || "Every space we design carries memory forward — alive with meaning.";
