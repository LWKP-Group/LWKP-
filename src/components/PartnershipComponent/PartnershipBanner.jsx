...
export default function PartnershipBanner() {
  ...
  const partnership = posts[0];
  const bgImage = partnership?.featured_image;
  const title = partnership?.title?.rendered || "partnership";
  const description =
    partnership?.acf?.main_description || "Every space we design carries memory forward — alive with meaning.";
