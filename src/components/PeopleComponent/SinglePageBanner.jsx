...
export default function SinglePageBanner() {
  ...
  const people = posts[0];
  const bgImage = people?.acf?.single_page_image || "";
  const title = people?.title?.rendered || "People";
  const description =
    people?.acf?.main_description || "Every space we design carries memory forward — alive with meaning.";
