...
import { fetchpurposePosts, selectpurposePosts, selectpurposeLoading } from "@/store/slices/purposeSlice";
...
export default function PurposeBanner() {
  ...
  const purpose = posts[0];
  const bgImage = purpose?.featured_image || "";
  const title = purpose?.title?.rendered || "Purpose";
  const description = purpose?.acf?.tagline || "Every space we design carries memory forward — alive with meaning.";
