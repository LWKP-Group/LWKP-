...
import { fetchMediaPosts, selectMediaPosts, selectMediaLoading } from "@/store/slices/mediaSlice";
import { shuffleArray } from "@/lib/shuffleArray";
import { limitWords } from "@/lib/shuffleArray";

export default function RelatedMediaPost() {
  ...
  useEffect(() => {
    if (!posts || posts.length === 0) {
      dispatch(fetchMediaPosts({ page: 1 }));
    }
  }, [dispatch]);
        ...
        {randomPosts.map((post) => {
          ...
          return (
            <div className="col-sm-6 mb-4" key={post.id}>
              ...
              <Link href={`/Media/${slug}`}> {lang === "ch" ? "阅读更多 → " : "Read More → "}</Link>
            </div>
          );
        })}
