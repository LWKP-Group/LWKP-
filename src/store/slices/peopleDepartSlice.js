...
export const fetchpeopleDepart = createAsyncThunk("peopleDepart/fetch", async ({ page = 1 }, { getState }) => {
  const lang = getState().language?.currentLanguage || "en";

  const wpLang = lang === "ch" ? "zh-hans" : "en";

  const res = await fetch(
    `${process.env.NEXT_PUBLIC_WP_API}/people_departs?lang=${wpLang}&page=${page}&per_page=10&_embed`,
  );
