...
export const fetchProjectTypes = createAsyncThunk("projectTypes/fetch", async (_, { getState }) => {
  const lang = getState().language?.currentLanguage || "en";

  const wpLang = lang === "ch" ? "zh-hans" : "en";

  const res = await fetch(`${process.env.NEXT_PUBLIC_WP_API}/project_type?lang=${wpLang}&per_page=100`);
