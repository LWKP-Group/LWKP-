...
export const fetchMediaCategories = createAsyncThunk("mediaCategories/fetch", async (_, { getState }) => {
  const lang = getState().language?.currentLanguage || "en";

  const wpLang = lang === "ch" ? "zh-hans" : "en";

  const res = await fetch(`${process.env.NEXT_PUBLIC_WP_API}/media_category?lang=${wpLang}&per_page=100`);
