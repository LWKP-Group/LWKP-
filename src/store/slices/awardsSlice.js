...
export const fetchAwardsYears = createAsyncThunk("awards/fetchYears", async (_, { getState }) => {
  const lang = getState().language?.currentLanguage || "en";

  const wpLang = lang === "ch" ? "zh-hans" : "en";

  const res = await fetch(`${process.env.NEXT_PUBLIC_WP_API}/awards?lang=${wpLang}&per_page=100&_fields=acf`);
...
export const fetchawardsPosts = createAsyncThunk(
  "awards/fetchPosts",
  async ({ page = 1, keyword = "", year = "" }, { getState }) => {
    ...
    let url =
      `${process.env.NEXT_PUBLIC_WP_API}/awards` +
      `?lang=${wpLang}` +
      `&page=${page}` +
      `&per_page=12` +
      `&orderby=date` +
    ...
    if (year) {
      url += `&awards_year=${encodeURIComponent(year)}`;
    }
